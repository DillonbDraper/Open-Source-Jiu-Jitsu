...
`
  ...
  // ../_build/dev/phoenix-colocated/fos_bjj/index.js
  var imp_nbxw623t = {};
  imp_nbxw623t["FosBjjWeb.VideoShowComponent.YouTubeSeeker"] = a25xont4oyukkp7vvlet3nnkgq_default;

  // js/app.js
  var import_topbar = __toESM(require_topbar());

  // vendor/collapsible.js
  var Collapsible = {
    mounted() {
      this.initElements();
  ...
  var Carousel = {
    ...
    setupState() {
      ...
      this.activeIndex = /^\d+$/.test(rawIndex) ? Math.max(1, Math.min(parseInt(rawIndex, 10), this.slides.length)) : 1;
      this.totalSlides = this.slides.length;
      this.classes = {
        activeSlide: this.sanitizeClassName(this.el.dataset.activeSlideClass) || "active-slide z-10",
        hiddenSlide: this.sanitizeClassName(this.el.dataset.hiddenSlideClass) || "opacity-0",
        activeIndicator: this.sanitizeClassName(this.el.dataset.activeIndicatorClass) || "active-indicator"
      };
      this.shouldAutoplay = this.el.dataset.autoplay === "true";
      const rawInterval = parseInt(this.el.dataset.autoplayInterval, 10);
      this.autoplayInterval = !isNaN(rawInterval) && rawInterval > 0 ? Math.min(rawInterval, 3e4) : 5e3;
      this.autoplayTimer = null;
      this.userInteractionDelay = 7e3;
    ...
    setupEventListeners() {
      this.eventHandlers = {
        ...
        mouseLeave: () => {
          this.isHovered = false;
          if (this.shouldAutoplay && !this.isTouching && this.el.dataset.pauseOnHover !== "false") {
            this.startAutoplay();
          }
        },
        touchStart: (e) => {
    ...
    handleKeyDown(e) {
      const hasFocus = this.el.contains(document.activeElement) || document.activeElement === this.el;
      if (!hasFocus) return;
    ...
    handlePrevClick(e) {
      const targetIndex = this.activeIndex > 1 ? this.activeIndex - 1 : this.totalSlides;
      this.navigateToSlide(targetIndex);
      e.stopPropagation();
    },
    handleNextClick(e) {
      const targetIndex = this.activeIndex < this.totalSlides ? this.activeIndex + 1 : 1;
      this.navigateToSlide(targetIndex);
      e.stopPropagation();
    },
    ...
    navigateToSlide(targetIndex) {
      if (!/^\d+$/.test(String(targetIndex)) || targetIndex < 1 || targetIndex > this.totalSlides) {
    ...
    startAutoplay() {
      ...
      this.autoplayTimer = setInterval(() => {
        const nextIndex = this.activeIndex < this.totalSlides ? this.activeIndex + 1 : 1;
    ...
    createLoadingOverlay(img) {
      ...
      overlay.className = "absolute inset-0 bg-gray-200 dark:bg-gray-800 animate-pulse flex items-center justify-center transition-opacity duration-300";
    ...
    sanitizeClassName(classString) {
      ...
      return classString.split(" ").map((name) => name.replace(/[^a-zA-Z0-9_\-]/g, "")).filter((name) => name.length > 0).join(" ");
    }
  };
  ...
  var Clipboard = {
    ...
    initializeElements() {
      ...
      this.triggerEl = this.triggerWrapper?.firstElementChild || this.triggerWrapper;
    ...
    initializeConfig() {
      ...
      this.timeout = !isNaN(timeoutValue) && timeoutValue > 0 ? timeoutValue : 2e3;
    ...
    sanitizeClassName(classNames) {
      ...
      return classNames.split(" ").filter((name) => /^[a-zA-Z0-9_\-]+$/.test(name)).join(" ") || "clipboard-success";
    },
    /**
    ...
    sanitizeText(text) {
      ...
      return text.slice(0, 100).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
    },
    /**
  ...
  var Combobox = {
    mounted() {
      ...
      if (!this.openButton.id) {
        this.openButton.id = `combobox-trigger-${Math.random().toString(36).substring(2, 9)}`;
      }
    ...
    openDropdown() {
      ...
      requestAnimationFrame(() => {
        ...
        Array.from(this.dropdownOptions).forEach(
          (opt) => opt.removeAttribute("data-combobox-navigate")
        );
        if (navigateTarget) {
    ...
    updateSingleSelectedDisplay() {
      ...
      if (selectedOption) {
        ...
        this.selectedDisplay.innerHTML = renderedOption ? renderedOption.innerHTML : selectedOption.textContent;
        if (renderedOption) {
    ...
    updateMultipleSelectedDisplay() {
      ...
      selectedOptions.forEach((option) => {
        ...
        closeBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="combobox-icon"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>';
    ...
    handleKeyDown(e) {
      const key = e.key;
      if (this.dropdown.hasAttribute("hidden") && document.activeElement === this.openButton) {
        if (key === " " || key === "Enter") {
          e.preventDefault();
          this.openDropdown();
          return;
      ...
      if (this.searchInput && document.activeElement === this.searchInput && !isNavigationKey && key.length === 1) {
        return;
      }
      const visibleOptions = Array.from(this.getDropdownOptions()).filter(
        (opt) => opt.style.display !== "none"
      ...
      let currentIndex = visibleOptions.findIndex(
        (opt) => opt.hasAttribute("data-combobox-navigate")
      );
      if (key === "ArrowDown") {
        e.preventDefault();
        e.stopPropagation();
        ...
        e.preventDefault();
        e.stopPropagation();
        currentIndex = currentIndex < 0 ? visibleOptions.length - 1 : (currentIndex - 1 + visibleOptions.length) % visibleOptions.length;
        this.navigateToOption(visibleOptions[currentIndex]);
        return;
      } else if (key === "Enter") {
        e.preventDefault();
    ...
    handleCharacterNavigation(char, options) {
      const matchingOptions = options.filter((opt) => {
        ...
        return valueAttr.toLowerCase().startsWith(char) || labelText.startsWith(char);
      });
      if (matchingOptions.length === 0) return;
      let currentIndex = matchingOptions.findIndex(
        (opt) => opt.hasAttribute("data-combobox-navigate")
      );
    ...
    handleDocumentClick(e) {
      if (!this.el.contains(e.target) && !(this.isPortalActive && this.portalContainer && this.portalContainer.contains(e.target)) && !this.isClickOnScrollbar(e)) {
        this.closeDropdown();
      }
    },
    ...
    destroyed() {
      ...
      if (this.boundOptionClickHandlers && this.boundOptionClickHandlers.length > 0) {
        this.boundOptionClickHandlers.forEach(({ btn, handler }) => {
          btn.removeEventListener("click", handler);
        });
      }
  ...
  var Floating = {
    ...
    cleanupDuplicateIds() {
      if (this.floatingContent && this.floatingContent.id) {
        ...
        duplicates.forEach((duplicate) => {
          if (duplicate !== this.floatingContent && duplicate.parentNode === document.body) {
            try {
    ...
    initElements() {
      this.floatingContent = this.el.querySelector("[data-floating-content]") || this.el.querySelector(".dropdown-content");
      this.trigger = this.el.querySelector("[data-floating-trigger]") || this.el.querySelector(".dropdown-trigger");
    },
    ...
    handleOutsideClick(e) {
      if (this.trigger && !this.trigger.contains(e.target) && (!this.floatingContent || !this.floatingContent.contains(e.target))) {
        this.hide();
      }
    },
    handleKeydown(e) {
    ...
    updatePosition() {
      ...
      if (this.smartPositioning && content.offsetHeight) {
        const { top: top3, bottom: bottom2, left: left2, right } = rect;
        const { innerHeight, innerWidth } = window;
        const height = content.offsetHeight;
        const width = content.offsetWidth;
        const spaceTop = top3;
        const spaceBottom = innerHeight - bottom2;
        const spaceLeft = left2;
      ...
      let top2, left;
      if (pos === "top") {
        top2 = rect.top + window.scrollY - content.offsetHeight - gap;
        left = (rect.left + rect.right) / 2 + window.scrollX;
        content.style.transform = "translateX(-50%)";
      } else if (pos === "bottom") {
        top2 = rect.bottom + window.scrollY + gap;
        left = (rect.left + rect.right) / 2 + window.scrollX;
        content.style.transform = "translateX(-50%)";
      } else if (pos === "left") {
        top2 = rect.top + window.scrollY + (rect.height - content.offsetHeight) / 2;
        left = this.isRTL ? rect.right + window.scrollX + gap : rect.left + window.scrollX - content.offsetWidth - gap;
        content.style.transform = "none";
      } else if (pos === "right") {
        top2 = rect.top + window.scrollY + (rect.height - content.offsetHeight) / 2;
        left = this.isRTL ? rect.left + window.scrollX - content.offsetWidth - gap : rect.right + window.scrollX + gap;
        content.style.transform = "none";
      }
    ...
    hide() {
      if (!this.floatingContent) return;
      const isCurrentlyVisible = this.floatingContent.classList.contains("show-dropdown");
      this.setVisibility(this.floatingContent, false);
      if (this.enableAria && isCurrentlyVisible) {
        setTimeout(() => {
          if (this.floatingContent && !this.floatingContent.classList.contains("show-dropdown")) {
  ...
  var GalleryFilter = {
    ...
    _isItemVisible(item, category) {
      ...
      const itemCategories = item.dataset.category.split(",").map((c) => c.trim().toLowerCase());
  ...
  var galleryFilter_default = GalleryFilter;

  // vendor/scrollArea.js
  var ScrollArea = {
    mounted() {
      const { el } = this;
    ...
    updateAxis({ contentSize, clientSize, scrollPos, thumb, scrollbar, axis }) {
      ...
      if (thumb) {
        const thumbSize = axis === "vertical" ? thumb.offsetHeight || 20 : thumb.offsetWidth || 20;
    ...
    onThumbYPointerDown(e) {
      ...
      this.boundThumbYPointerMove = (e2) => this.onThumbYPointerMove(e2);
    ...
    onThumbXPointerDown(e) {
      ...
      this.boundThumbXPointerMove = (e2) => this.onThumbXPointerMove(e2);
  ...
  var Sidebar = {
    ...
    getConfig(sidebar) {
      ...
      return {
        expandedWidth: this.sanitizeCssValue(expandedWidth) || this.defaults.expandedWidth,
        collapsedWidth: this.sanitizeCssValue(collapsedWidth) || this.defaults.collapsedWidth,
  ...
  var Components = {
    ...
    GalleryFilter: galleryFilter_default,
    ScrollArea: scrollArea_default,
    Sidebar: sidebar_default
  };
  var mishka_components_default = Components;
