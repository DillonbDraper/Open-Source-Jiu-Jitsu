defmodule FosBjj.Accounts.UserRoleHelpersTest do
  ...
  test "coach_or_admin?/1 requires verified coach or admin" do
    verified_admin = struct(User, role_name: "admin", confirmed_at: DateTime.utc_now())
    verified_coach = struct(User, role_name: "coach", confirmed_at: DateTime.utc_now())
    verified_student = struct(User, role_name: "student", confirmed_at: DateTime.utc_now())
  ...
  test "coach_application_eligible?/1 respects belt and experience" do
    assert User.coach_application_eligible?(%{
             bjj_belt: :black,
             other_high_level_experience: false
           })

    assert User.coach_application_eligible?(%{bjj_belt: "black"})
    assert User.coach_application_eligible?(%{other_high_level_experience: true})

    refute User.coach_application_eligible?(%{
             bjj_belt: :blue,
             other_high_level_experience: false
           })
  end
